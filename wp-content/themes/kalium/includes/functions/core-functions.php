...
/**
 * Check if style component should be enqueued.
 *
 ...
 * @since 3.4
 */
function kalium_should_enqueue( $component ) {
	$result       = false;
	...
	/**
	 * Check if any of widget (or array of widget ids) type exists.
	 *
	 ...
	 * @return bool
	 */
	$elementor_widget_type_exists = function ( $widget_types, $elements_data ) use ( & $elementor_widget_type_exists ) {
		if ( is_string( $widget_types ) ) {
			$widget_types = [ $widget_types ];
		}

		if ( is_array( $widget_types ) && is_array( $elements_data ) ) {
			foreach ( $widget_types as $widget_id ) {
				$is_regex = false !== strpos( $widget_id, '*' );

				foreach ( $elements_data as $item ) {
					$widget_type = kalium_get_array_key( $item, 'widgetType' );

					if ( $widget_id === $widget_type || ( $is_regex && preg_match( '/' . str_replace( '*', '[\w\-]*', $widget_id ) . '/', $widget_type ) ) ) {
						return true;
					} else if ( isset( $item['elements'] ) ) {
						if ( $elementor_widget_type_exists( $widget_id, $item['elements'] ) ) {
							return true;
						}
					}
				}
	...
	// Portfolio style
	if ( 'portfolio' === $component ) {

		// Only if Portfolio plugin is active
		if ( kalium()->is->portfolio_plugin_active() ) {
			...
			// Portfolio archive and single page
			if ( is_post_type_archive( 'portfolio' ) || is_singular( 'portfolio' ) || is_tax( [ 'portfolio_category', 'portfolio_tag' ] ) ) {
				$result = true;
			} // Portfolio in shortcodes
			else if ( $post_content && preg_match( '/\[' . implode( '|', $shortcodes ) . '/', $post_content ) ) {
				$result = true;
			} // Portfolio page template
			else if ( is_page_template( 'portfolio.php' ) ) {
				$result = true;
			} // Portfolio in elementor
			else if ( $post_id && kalium()->elementor->is_built_with_elementor( $post_id ) ) {
				$elements_data = kalium()->elementor->get_elements_data( $post_id );

				if ( $elementor_widget_type_exists( 'kalium-portfolio-items-legacy', $elements_data ) ) {
					$result = true;
				}
			}
		}
	} // WooCommerce style
	else if ( 'woocommerce' === $component ) {
		...
		if ( kalium()->is->woocommerce_active() ) {
			...
			// Shop archive and single page
			if ( is_woocommerce() ) {
				$result = true;
			} // Pages that contain WooCommerce shortcodes
			else if ( $post_content && preg_match( '/\[' . implode( '|', $shortcodes ) . '/', $post_content ) ) {
				$result = true;
			} // WooCommerce in elementor
			else if ( $post_id && kalium()->elementor->is_built_with_elementor( $post_id ) ) {
				$elements_data = kalium()->elementor->get_elements_data( $post_id );

				if ( $elementor_widget_type_exists( '*woocommerce*', $elements_data ) ) {
					$result = true;
				}
			}
		}
	...
	/**
	 * Hook: kalium_should_enqueue.
	 *
	 * @param bool   $result
	 * @param string $component
	 */
	return apply_filters( 'kalium_should_enqueue', $result, $component );
}
...
function kalium_add_inline_script( $handle, $data, $position = 'after' ) {
	kalium()->enqueue->add_inline_script( $handle, $data, $position );
}
...
/**
 * Get attachment image, the Kalium way, no lazy loading.
 *
 * @param int        $attachment_id
 * @param string     $size
 * @param null|array $atts
 ...
 * @return string
 */
function kalium_get_attachment_image_raw( $attachment_id, $size = 'thumbnail', $atts = null, $placeholder_atts = null ) {
	return kalium()->images->get_image( $attachment_id, $size, $atts, $placeholder_atts, false );
}
...
function kalium_js_assets_enqueue_mapping() {
	...
	// Libraries available to enqueue
	$libraries = [
		'light-gallery',
		'videojs',
	];
	...
	foreach ( $libraries as $library ) {
		$enqueue_items = kalium()->enqueue->get_enqueue_items( $library );
	...
	kalium_define_js_variable( [
		...
		'require'       => '~function(e){var t=e instanceof Array?e:[e],r=function(e){var t,t;e.match(/\.js(\?.*)?$/)?(t=document.createElement("script")).src=e:((t=document.createElement("link")).rel="stylesheet",t.href=e);var r=!1,a=jQuery("[data-deploader]").each((function(t,a){e!=jQuery(a).attr("src")&&e!=jQuery(a).attr("href")||(r=!0)})).length;r||(t.setAttribute("data-deploader",a),jQuery("head").append(t))},a;return new Promise((function(e,a){var n=0,c=function(t){if(t&&t.length){var a=t.shift(),n=a.match(/\.js(\?.*)?$/)?"script":"text";jQuery.ajax({dataType:n,url:a,cache:!0}).success((function(){r(a)})).always((function(){a.length&&c(t)}))}else e()};c(t)}))};'
	] );
}
...
function kalium_get_social_networks_list() {
	...
	foreach ( $social_networks_ordered as $old_social_network_id => $title ) {
		...
		// Phone type
		if ( 'phone' === $social_network_id ) {
			$social_network['data']['is_phone'] = true;
		}
