					...
					switch ( $value ) {
						...
						case 'raw-text':
							kalium()->helpers->build_dom_element( [
								...
								'content'    => do_shortcode( wp_kses_post( $option_text ) ),
								'echo'       => true,
							] );
							break;
						...
						case 'date-time':
							...
							// Date format
							$date_format = kalium_get_array_key( $options, 'date_format' );
						...
						case 'breadcrumb':
							...
							if ( kalium()->is->breadcrumb_navxt_active() ) {
								...
								if ( $option_separator ) {
									kalium_append_custom_css( '.breadcrumb.' . $breadcrumb_id . ' li:after', [
						...
						case 'woocommerce-mini-cart':
							if ( function_exists( 'kalium_woocommerce_cart_menu_icon' ) ) {
								$option_title = kalium_get_array_key( $options, 'title' );

								kalium_woocommerce_cart_menu_icon( $option_skin, $option_align, $option_title );
							}
							break;
