...
class Laborator_System_Status {
	...
	public static function display_php_version() {
		...
		if ( version_compare( $php_version, self::MIN_PHP_VERSION, '>=' ) ) {
			...
			echo sprintf( '%s - We recommend using PHP version 7.4 or above for greater performance and security. <a href="%s" target="_blank" rel="noreferrer noopener">How to update your PHP version &raquo;</a>', $php_version_display, 'https://documentation.laborator.co/kb/general/how-to-upgrade-php-version-and-increase-server-limits/' );
		}
	}
	...
	public static function display_php_max_execution_time() {
		...
		if ( $max_execution_time >= 30 ) {
			...
			if ( $max_execution_time < $demo_content_execution_time ) {
				echo sprintf( '<strong>%s</strong> - We recommend setting max execution time at least <strong>%d</strong> when you are importing demo content. See: <a href="%s" target="_blank" rel="noopener noreferrer">Increasing max execution to PHP</a>', self::red_text( $max_execution_time ), $demo_content_execution_time, 'http://codex.wordpress.org/Common_WordPress_Errors#Maximum_execution_time_exceeded' );
			} else {
