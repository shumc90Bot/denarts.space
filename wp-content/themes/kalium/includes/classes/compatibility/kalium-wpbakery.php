...
class Kalium_WPBakery {
    ...
    public function _vc_theme_plugin_mode_option() {
        ...
        $field_callback = function() use ( $theme_plugin_mode_field_name ) {
            ...
            ?>
            <label>
                <input type="checkbox"<?php checked( $checked ); disabled( $disabled ?? false ); ?> value="1" id="<?php echo esc_attr( $theme_plugin_mode_field_name ); ?>" name="<?php echo esc_attr( $theme_plugin_mode_field_name ); ?>">
     ...
     * @return string
     */
    public function _vc_hide_notices( $notices ) {
        try {
            ...
            if ( is_array( $notices_arr ) ) {
                foreach ($notices_arr as $i => $notice) {
                    ...
                    $fields_content = kalium_get_array_key($notice, 'title') . kalium_get_array_key($notice, 'button_text');
